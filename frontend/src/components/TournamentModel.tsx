  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="w-full max-w-md rounded-lg overflow-hidden shadow-xl relative">
        <div className={`absolute inset-0 bg-gradient-to-r from-black via-transparent to-black opacity-0 transition-opacity duration-500 ${isAnimating ? 'opacity-30' : ''} group-hover:opacity-30 z-10 pointer-events-none`}></div>
        
        <div className={`absolute w-full h-16 bg-white/10 -skew-y-12 -top-20 blur-xl ${isAnimating ? 'animate-lightSweep' : ''} group-hover:animate-lightSweep pointer-events-none`}></div>
      ...
      <Modal>
        <ModalTrigger 
          className="bg-black dark:bg-white dark:text-black text-white flex justify-center items-center group rounded-full w-48 h-16 shadow-xl relative overflow-hidden"
          onMouseEnter={() => setIsAnimating(true)}
          onMouseLeave={() => setIsAnimating(false)}
        >
          <div className={`absolute inset-0 bg-blue-500/20 blur-md transition-opacity duration-300 opacity-0 group-hover:opacity-100 ${isAnimating ? 'opacity-100' : ''}`}></div>
          
          <div className={`absolute w-16 h-full bg-white/20 -skew-x-12 -left-32 group-hover:left-64 transition-all duration-700 ${isAnimating ? 'left-64' : '-left-32'}`}></div>
          
          <div className="flex items-center justify-center gap-2 z-10">
            <span className="font-bold text-xl transition-all duration-300 group-hover:text-white">Tournaments</span>
            
            <div className={`text-3xl ml-1 transition-all duration-500 ${isAnimating ? 'scale-125 animate-pulse' : ''} group-hover:scale-125 group-hover:animate-pulse`}>
              🏆
            </div>
          </div>
          
          <div className={`absolute inset-0 rounded-full border-2 border-blue-400/0 group-hover:border-blue-400/70 transition-all duration-500 ${isAnimating ? 'border-blue-400/70 scale-105' : ''} group-hover:scale-105`}></div>
