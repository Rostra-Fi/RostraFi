...
const initializeGame = async (req, res, next) => {
  try {
    ...
    let game = await CandyCrush.findOne({ walletAddress });
...
const updateLevelProgress = async (req, res, next) => {
  try {
    ...
    console.log('Updating level progress for:', walletAddress, levelNumber);

    // Find the game for the user
    const game = await CandyCrush.findOne({ walletAddress });
    console.log('Game found:', game);

    if (!game) {
      return next(
        new ErrorResponse(
          `Candy Crush game not found for user ${walletAddress}`,
          404,
        ),
      );
    }
