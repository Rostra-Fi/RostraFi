...
const initializeGame = async (req, res, next) => {
  try {
    ...
    let game = await Platformer.findOne({ walletAddress });

    if (game) {
      return next(
        new ErrorResponse(
          `Platformer game already exists for user ${walletAddress}`,
          400,
        ),
      );
    }
...
const updateLevelProgress = async (req, res, next) => {
  try {
    ...
    const game = await Platformer.findOne({ walletAddress });

    if (!game) {
      return next(
        new ErrorResponse(
          `Platformer game not found for user ${walletAddress}`,
          404,
        ),
      );
    }
