...
const initializeGame = async (req, res, next) => {
  try {
    ...
    let game = await SpaceInvaders.findOne({ walletAddress });

    if (!game) {
      // Create new game
      game = await SpaceInvaders.create({ walletAddress });
...
const recordGameSession = async (req, res, next) => {
  try {
    ...
    const game = await SpaceInvaders.findOne({ walletAddress });

    if (!game) {
      return next(
        new ErrorResponse(
          `Space Invaders game not found for user ${walletAddress}`,
          404,
        ),
      );
    }
